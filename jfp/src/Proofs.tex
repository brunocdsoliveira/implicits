...
then
\begin{equation*}
  \ares{\tenv, \ie{\ruleset}{\bar{x}}}{\type}{E'}
...
As shown in Figure~\ref{fig:unambenv}, a typing environment is unambiguous when all its type
variables and implicit entries have unambiguous types. This definition will be used thoughout
this section.
...
%===============================================================================
  ...
  hypothesis is also derived using rule \rref{L-RuleMatch}.
...
The next two proofs make use of the auxiliary inductive definition R, shown in
Figure~\ref{fig:R}. When performing a type substitution $[\suty/\alpha]$ under an environment
that is represented as the concatenation of two environments, $\tenv = \tenv_1,\tenv_2$,
this definition separates between the two cases $\alpha\in\tenv_1$ and $\alpha\in\tenv_2$ and
specifies the resulting environment after the substitution in terms of the $\tenv_1$ and
$\tenv_2$ environments. The first three arguments of R, when concatenated, represent the
initial complete environment $\tenv$, while the next two, when concatenated, represente the
updated environment after the substitution.
...
\begin{equation*}
  ...
  %\theta(\tenv,\alpha,\tenv'); \theta(\rulet)\leadsto x \ivturns \theta(\tau)\leadsto E; \Sigma
\end{equation*}
...
\begin{equation*}
  ...
  %\theta'(\tenv,\tenv'[\suty/\alpha]); \theta'(\rulet[\suty/\alpha])\leadsto x \ivturns \theta'(\tau[\suty/\alpha])\leadsto E; \Sigma
\end{equation*}
...
%$\bar{\alpha},\alpha,\bar{\alpha}';\tenv,\alpha,\tenv';\tenv'', x : \rulet  \ivturns \type \leadsto E$ \ \\
%===============================================================================
...
%$\tenv,\alpha,\tenv'; \type \leadsto E \ivturns \type \leadsto E; \epsilon$ \ \\
%===============================================================================
...
%$\tenv,\alpha,\tenv'; \rulet_1 \iarrow \rulet_2 \leadsto E \ivturns \type \leadsto E'; \Sigma, \rulet_1 \leadsto x$ \ \\
%===============================================================================
...
\end{equation*}
  ...
  From these two facts we conclude by rule \rref{M-TApp}, reasoning modulo
...
\begin{proof}
\klara{Don't forget to update with changes in the typing relation!}\\
...
Using the two obtained results in rule \rref{Ty-TApp}, we get
\begin{equation*}
...
\end{equation*}
Passing the second and third assumption of rule \rref{Ty-IAbs} together with the second
hypothesis of this theorem to Lemma~\ref{lemma:unamb_subst:0}, we get
\begin{equation*}
...
By using the three obtained results and the freshness assumption of rule \rref{Ty-IAbs}
...
%===============================================================================
From the first assumption of rule \rref{Ty-Query}, the second hypothesis of this theorem and
Lemma~\ref{lemma:coherence:0} \klara{It is unclear to the reader why I use
Lemma~\ref{lemma:coherence:0} and not Lemma~\ref{lemma:substitution}.}
we get
\begin{equation*}
...
\end{equation*}
Finally, passing the second and third assumptions of rule \rref{Ty-Query} and the second
hypothesis of this theorem to Lemma~\ref{lemma:unamb_subst:0}, results in
\begin{equation*}
\unambig{}{\rulet[\suty/\alpha]}
\end{equation*}
Then, we can instantiate rule \rref{Ty-Query} with the above three obtained results to reach
...
By case analysis, this well-scoping judgment can only hold as the conclusion of rule \rref{WF-RulTy}. From its premises, we get
\begin{equation*}
\wfty{\tenv,\alpha,\tenv'}{\rulet_1}
\end{equation*}

Passing the first assumption of rule \rref{UA-IAbs}, the obtained well-scoping result above and
