...
Figure~\ref{fig:R}. When performing a type substitution
$\vsubst{\tenv_x,\tenv_y}{[\suty/\alpha]}$, this definition separates between the two cases
$\alpha\in\tenv_x$ and $\alpha\in\tenv_y$. The first case is covered by rule (R-2), where
$\tenv_x = \tenv,\alpha,\tenv_1'$, and the second by (R-1), with $\tenv_x = \tenv_1$.
The fourth argument represents $\tenv_x$ and the fifth analayzes the updated $\tenv_x$
environment that results from performing the substitution.
