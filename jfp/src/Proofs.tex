...
is necessary to handle the multiple substitutions that rule \rref{L-Match} uses.

\begin{center}
\begin{tikzpicture}[node distance=1cm, auto,]
%nodes
\lnode{DR}{mainres}{lemma:s23:4}{Det. resolution soundness}
\lnode{DFR}{right=0.8cm of DR}{lemma:s23:3}{Det. focusing soundness}
\lnode{DLR}{right=0.8cm of DFR}{lemma:s23:2}{Det. lookup soundness}
\lnode{SDMR}{below=of DLR}{lemma:s23:1}{Det. matching soundness under subst.}
\lnode{DMR}{right=0.8cm of SDMR}{lemma:s23:5}{Det. matching soundness}
\lnode{SUB}{left= 0.8cm of SDMR}{lemma:substitution}{Substitution}
%edges
...
\draw[pil] (DLR.south) to                  (SDMR.north);
\draw[pil] (SDMR.east) to                  (DMR.west);
\draw[pil] (SDMR.west) to                  (SUB.east);
\end{tikzpicture}
...
\end{myequation*}%
  Then the fourth hypothesis was either derived from rule \rref{L-Match},
  or from rule \rref{L-NoMatch}. However, the hypothesis of the latter is
  not satisfied: $\epsilon;E_1;\Sigma_1$ forms a counter-example. Hence, using rule \rref{L-Match}
...
%===============================================================================
  ...
  Then the fourth hypothesis was either derived from rule \rref{L-Match},
  or from rule \rref{L-NoMatch}. However, the hypothesis of the former is
...
%\begin{minipage}[t]{\textwidth}
The proofs of this section concern stability of the deterministic resolution judgment, as well
as stability of typing. The first part, similarly to Section~\ref{proof:determinism}, is split
into subproofs for each auxiliary judgment (focusing, lookup and matching) of the deterministic
...
\lnode{DMR}{right=0.8cm of DLR}{lemma:coherence:3}{Matching judgment is stable}
...
\end{myequation*}
  By combining these two observations with rule \rref{L-Match} we obtain the first desired result
\begin{myequation*}
...
\begin{myequation*}
  \dlres{\bar{\alpha},\bar{\alpha}'}{\tenv,\tenv'[\suty/\alpha]}{\tenv'''}{\type[\suty/\alpha]}{E'[|\suty|/\alpha]}
\end{myequation*}
  With rule \rref{L-NoMatch} we combine these two observations into the desired first result
\begin{myequation*}
...
This section concerns properties of the unification algorithm that are required by the proofs of
...
Then, using Lemma~\ref{lemma:asoundness:1} and rule \rref{L-Match} we conclude
\begin{myequation*}
...
\begin{myequation*}
\validsubst{\bar\alpha}{\tenv}{\theta}~~~~\text{and}~~~~
%\dom(\theta) \subseteq \bar{\alpha}:
\dmres{\theta(\tenv)}{\theta(\rulet)}{x}{\theta(\type)}{E}{\Sigma}
\end{myequation*}
  Hence, with rule \rref{L-NoMatch} we conclude
\begin{myequation*}
...
%We assume that the judgment is decorated with an additional output argument, the
%substitution for the $\bar{\alpha}$ type variables. This argument is actually computed when
%rule~\rref{Alg-M-Simp} is reached and then it is propagated through the rest of the rules.
...
\begin{lemma}\label{lemma:asoundness:3}
  If 
\begin{myequation*}
%\admress{\bar\alpha}{\tenv}{\rulet}{E}{\Sigma}{\type}{E'}{\Sigma'}{\theta}
...
\end{myequation*}
%where the meta-variable $\Sigma''$ from the theorem statement has been instantiated to $\epsilon$.\\
...
\begin{myequation*}
  \validsubst{\bar\alpha}{\tenv,\envi{\rulet}{x}}{\theta}
\end{myequation*}
and
\begin{myequation*}
...
\begin{myequation*}
  \validsubst{\bar\alpha}{\tenv}{\theta}
\end{myequation*}
By rule \rref{M-IApp}, applied on the second result of the induction hypothesis, we may conclude
\begin{myequation*}
...
%$\admress{\bar\alpha}{\tenv}{\tall{\alpha}{\rulet}}{E}{\Sigma}{\type}{E'}{\Sigma'}{[\suty/\alpha]\cdot \theta}$\ \\
...
refined to
\begin{myequation*}
\dmres{\theta'(\tenv)}{\theta'(\rulet[[\suty/\alpha]])}{\etrans\theta'(\mmapp{E}{\etrans{\suty}})}{\theta'(\type)}{E'}{\Sigma''}
\end{myequation*}
Hence, it follows from rule \rref{M-TApp} and the type well-formedness judgment for \(\suty\)
that
\begin{myequation*}
...
\lnode{AF}{below=of AR}{lemma:acompleteness:1}{Algorithmic focusing is complete wrt det. focusing}
\lnode{AL}{right=1.33 of AF}{lemma:acompleteness:2}{Algorithmic lookup is complete wrt det. lookup}
\lnode{KL}{right=of AR}{lemma:acompleteness:4}{Alg. stability implies alg. matching}
\lnode{AM}{right=of AL}{lemma:acompleteness:3}{Algorithmic matching is complete wrt det. matching}
\lnode{AMC}{below=of AL}{lemma:asoundness:3}{Algorithmic matching is sound wrt det. matching}
\anode{MIS}{right=of KL}{axiom:mgu_property:2}{Unification algorithm matches}
\lnode{UAC}{right=of AMC}{lemma:mgu_property:1}{Unification algorithm unifies}
%edges
\draw[pil] (AR.south) to                  (AF.north);
\draw[pil] (AF.north) to [out=30,in=150]  (AL.north);
\draw[pil] (AL.south) to [out=210,in=330] (AF.south);
\draw[pil] (AL.north) to [out=30,in=150]  (AM.north);
\draw[pil] (AL.north) to                  (KL.south);
\draw[pil] (AM.north) to                  (MIS.south);
\draw[pil] (AL.south) to                  (AMC.north);
\draw[pil] (AMC.east) to                  (UAC.west);
\end{tikzpicture}
\end{center}\vspace{0.5cm}
...
\end{myequation*}
Then, using Lemma~\ref{lemma:acompleteness:1} and rule \rref{Alg-L-Match} we conclude
\begin{myequation*}
...
\end{myequation*}
  Hence with rule \rref{Alg-L-NoMatch} we conclude
\begin{myequation*}
  \adlres{\bar\alpha}{\tenv}{\tenv',\envi{\rulet}{x}}{\type}{E'}
\end{myequation*}
...
\begin{myequation*}
  ...
  \validsubst{\bar\alpha}{\tenv}{\theta_1}
\end{myequation*}
  then
\begin{myequation*}
  \coherent{\bar\alpha}{\tenv}{\rulet}{\type}
...
can consider $\theta'=[\suty/\alpha]\cdot\theta_1$. By instantiating rule \rref{S-Cons} with the
theorem's second hypothesis and the second premise of rule \rref{M-TApp}, we have
\begin{myequation*}
  \validsubst{\bar\alpha,\alpha}{\tenv,\alpha}{\theta'}
\end{myequation*}
