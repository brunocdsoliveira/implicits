...
%nodes
\node[punkt, mainres] (DR) {Lemma~\ref{lemma:determinism:0}: \\
...
%nodes
\node[punkt, mainres] (DR) {Lemma~\ref{lemma:coherence:0}: \\
             \it Deterministic resolution is stable};
\node[punkt, right=0.8cm of DR] (DFR) {Lemma~\ref{lemma:coherence:1}: \\
             \it Focusing judgment is stable};
\node[punkt, right=0.8cm of DFR] (DLR) {Lemma~\ref{lemma:coherence:2}: \\
             \it Lookup judgment is stable};
\node[punkt, right=0.8cm of DLR] (DMR) {Lemma~\ref{lemma:coherence:3}: \\
             \it Matching judgment is stasble};
\node[punkt, below=of DR] (TP) {Lemma~\ref{lemma:typing_stability}: \\
             \it Typing preservation under type substitution};
\node[punkt, mainres, below=of DFR] (TR) {Lemma~\ref{lemma:tapp_reduction}: \\
             \it Typing is stable under type application reduction};
\node[punkt, below=of DLR] (US) {Lemma~\ref{lemma:unamb_subst:0}: \\
             \it Unambiguity preservation under type substitution};
\node[punkt, below=of DMR] (UAS) {Lemma~\ref{lemma:unamb_subst:1}: \\
             \it Unambiguity preservation under type substitution};
%edges
\draw[pil] (DR.north) to [out=30,in=150] (DFR.north);
\draw[pil] (DFR.north) to [out=30,in=150] (DLR.north);
...
\begin{tikzpicture}[node distance=1.3cm, auto,]
%nodes
\node[punkt, mainres] (AR) {Lemma~\ref{lemma:asoundness:0}:\\
             \it Algorithmic resolution is sound wrt det. resolution};
\node[punkt, below=of AR] (AF) {Lemma~\ref{lemma:asoundness:1}: \\
             \it Algorithmic focusing is sound wrt to det. focusing};
\node[punkt, right=of AF] (AL) {Lemma~\ref{lemma:asoundness:2}: \\
             \it Algorithmic lookup is sound wrt to det. lookup};
\node[punkt, above=of AL] (AM) {Lemma~\ref{lemma:asoundness:3}: \\
             \it Algorithmic matching is sound wrt to det. matching};
\node[punkt, below=of AL] (AMC) {Lemma~\ref{lemma:acompleteness:3}: \\
             \it Algorithmic matching is complete wrt to det. matching};
\node[punkt, right=of AL] (MIS) {Lemma~\ref{lemma:asoundness:5}: \\
             \it Algorithmic matching implies alg. stability};
\node[punkt, right=of AM] (UAC) {Lemma~\ref{lemma:asoundness:6}: \\
             \it Unification algorithm unifies};
%edges
...
\begin{tikzpicture}[node distance=1.3cm, auto,]
%nodes
\node[punkt, mainres] (AR) {Lemma~\ref{lemma:acompleteness:0}:\\
             \it Algorithmic resolution is complete wrt det. resolution};
\node[punkt, right=0.8cm of AR] (AF) {Lemma~\ref{lemma:acompleteness:1}: \\
             \it Algorithmic focusing is complete wrt to det. focusing};
\node[punkt, right=0.8cm of AF] (AL) {Lemma~\ref{lemma:acompleteness:2}: \\
             \it Algorithmic lookup is complete wrt to det. lookup};
\node[punkt, right=0.8cm of AL] (AM) {Lemma~\ref{lemma:acompleteness:3}: \\
             \it Algorithmic matching is complete wrt to det. matching};
\node[punkt, below=of AL] (AMC) {Lemma~\ref{lemma:asoundness:3}: \\
             \it Algorithmic matching is sound wrt to det. matching};
\node[punkt, below=of AM] (MIS) {Lemma~\ref{lemma:acompleteness:5}: \\
             \it Unification algorithm produces most general unifier};
%\node[punkt, right=of AM] (UAC) {Lemma~\ref{lemma:asoundness:6}: \\
%             \it Unification algorithm unifies};
%edges
...
$\theta_2\sqsubseteq\theta_1$, iff there is a $\theta'$ such that $\theta_2\cdot\theta' = \theta_1$.
\end{definition}
