...
%nodes
\dnode{DR}{mainres}{lemma:determinism:0}{Deterministic resolution judgment is deterministic}
\dnode{DFR}{right=0.8cm of DR}{lemma:determinism:1}{Focusing judgment is deterministic}
\dnode{DLR}{right=0.8cm of DFR}{lemma:determinism:2}{Lookup judgment is deterministic}
\dnode{DMR}{right=0.8cm of DLR}{lemma:determinism:3}{Matching judgment is deterministic}
%edges
...
%nodes
\dnode{DR}{mainres}{lemma:coherence:0}{Deterministic resolution is stable}
\dnode{DFR}{right=0.8cm of DR}{lemma:coherence:1}{Focusing judgment is stable}
\dnode{DLR}{right=0.8cm of DFR}{lemma:coherence:2}{Lookup judgment is stable}
\dnode{DMR}{right=0.8cm of DLR}{lemma:coherence:3}{Matching judgment is stasble}
\dnode{TP}{below=of DR}{lemma:typing_stability}{Typing preservation under type substitution}
\dnode{TR}{mainres, below=of DFR}{lemma:tapp_reduction}{Typing is stable under type application reduction}
\dnode{US}{below=of DLR}{lemma:unamb_subst:0}{Unambiguity preservation under type substitution}
\dnode{UAS}{below=of DMR}{lemma:unamb_subst:1}{Unambiguity preservation under type substitution}
%edges
\draw[pil] (DR.north)  to [out=30,in=150]  (DFR.north);
\draw[pil] (DFR.north) to [out=30,in=150]  (DLR.north);
\draw[pil] (DLR.south) to [out=210,in=330] (DFR.south);
...
corresponding judgments. Interestingly, the proof for algorithmic lookup soundness also uses
completeness of the algorithmic matching judgment. Two additional auxiliary lemmata are required
for the algorithmic lookup and matching soundness.

\begin{center}
\begin{tikzpicture}[node distance=1.3cm, auto,]
%nodes
\dnode{AR}{mainres}{lemma:asoundness:0}{Algorithmic resolution is sound wrt det. resolution}
\dnode{AF}{below=of AR}{lemma:asoundness:1}{Algorithmic focusing is sound wrt det. focusing}
\dnode{AL}{right=of AF}{lemma:asoundness:2}{Algorithmic lookup is sound wrt det. lookup}
\dnode{AM}{right=of AR}{lemma:asoundness:3}{Algorithmic matching is sound wrt det. matching}
\dnode{AMC}{below=of AL}{lemma:acompleteness:3}{Algorithmic matching is complete wrt det. matching}
\dnode{MIS}{right=of AL}{lemma:asoundness:5}{Algorithmic matching implies alg. stability}
\dnode{UAC}{right=of AM}{lemma:asoundness:6}{Unification algorithm unifies}
%edges
\draw[pil] (AR.south) to                  (AF.north);
\draw[pil] (AF.north) to [out=30,in=150]  (AL.north);
\draw[pil] (AL.south) to [out=210,in=330] (AF.south);
\draw[pil] (AL.north) to                  (AM.south);
\draw[pil] (AM.east)  to                  (UAC.west);
\draw[pil] (AL.east)  to                  (MIS.west);
...
between the rule sets of the two judgements. Case \rref{Alg-R-Simp} follows
...
\end{equation*}
This follows from rule \rref{M-Simp}.
...
\begin{tikzpicture}[node distance=1.3cm, auto,]
%nodes
\dnode{AR}{mainres}{lemma:acompleteness:0}{Algorithmic resolution is complete wrt det. resolution}
\dnode{AF}{right=0.8cm of AR}{lemma:acompleteness:1}{Algorithmic focusing is complete wrt det. focusing}
\dnode{AL}{right=0.8cm of AF}{lemma:acompleteness:2}{Algorithmic lookup is complete wrt det. lookup}
\dnode{AM}{right=0.8cm of AL}{lemma:acompleteness:3}{Algorithmic matching is complete wrt det. matching}
\dnode{AMC}{below=of AL}{lemma:asoundness:3}{Algorithmic matching is sound wrt det. matching}
\dnode{MIS}{below=of AM}{lemma:acompleteness:5}{Unification algorithm produces most general unifier}
%edges
...
Cases \rref{IAbs} and \rref{R-TAbs} follow from the isomorphism
between the rule sets of the two judgements. Case \rref{R-Simp} follows
...
\begin{lemma}\label{lemma:acompleteness:3}
  If 
\begin{equation*}
\dmres{\theta_1(\tenv)}{\theta_1(\rulet)}{\etrans{\theta_1}(E)}{\theta_1(\type)}{E'}{\Sigma'}
\end{equation*}
  and
\begin{equation*}
...
\end{equation*}
  then
\begin{equation*}
  \admres{\bar\alpha}{\tenv}{\rulet}{E}{\Sigma}{\type}{\etrans{\theta_2}(E')}{\theta_2(\Sigma,\Sigma')}
\end{equation*}
  and
\begin{equation*}
  \dom{(\theta_2)} \subseteq \bar{\alpha}
\end{equation*}
  and
\begin{equation*}
...
$\dmres{\theta_1(\tenv)}{\theta_1(\type')}{\etrans{\theta_1}(E)}{\theta_1(\type)}{\etrans{\theta_1}(E)}{\epsilon}$ \quad with $\theta_1(\type) = \theta_1(\type')$ \\
...
$\dmres{\theta_1(\tenv)}{\theta_1(\rulet_1\To\rulet_2)}{\etrans{\theta_1}(E)}{\theta_1(\type)}{E'}{\ie{\theta_1(\rulet_1)}{x},\Sigma'}$\ \\
...
$\dmres{\theta_1(\tenv)}{\theta_1(\tall{\alpha}{\rulet})}{\etrans{\theta_1}(E)}{\theta_1(\type)}{E'}{\Sigma'}$\ \\
...
\begin{equation*}
  \dmres{\theta_1(\tenv)}{\theta_1(\rulet)[\suty/\alpha]}{\etrans{\theta_1}(\mmapp{E}{\etrans{\suty}})}{\theta_1(\type)}{E'}{\Sigma'}
\end{equation*}
...
\begin{equation*}
  \begin{array}{c}
  \theta'(\rulet) = \theta_1(\rulet)[\suty/\alpha]\\
  \theta'(\tenv,\alpha) = \theta_1(\tenv)\\
  \etrans{\theta'}(\mmapp{E}{\alpha}) = \etrans{\theta_1}(\mmapp{E}{\etrans{\suty}})
  \end{array}
\end{equation*}
In addition, since $\alpha$ does not appear free in the terms $\etrans{\theta_1}(E)$ and
$E'$, the type $\theta_1(\type)$ and the types in $\Sigma'$,
it follows that the substitution $[\suty/\alpha]$ does not affect them.
%\begin{equation*}
%  \begin{array}{c}
%    \theta'(\rulet) = \theta_1(\rulet)[\suty/\alpha]\\
%    \theta'(\tenv,\alpha) = \theta_1(\tenv)\\
%    \etrans{\theta'}(E) = \etrans{\theta_1}(E)\\
%    \theta'(\Sigma') = \theta_1(\Sigma')\\
%    \theta'(\type) = \theta_1(\type)
%  \end{array}
%\end{equation*}
The premise of the rule can, then, be rewritten as
\begin{equation*}
  \dmres{\theta'(\tenv,\alpha)}{\theta'(\rulet)}{\etrans{\theta'}(\mmapp{E}{\alpha})}{\theta'(\type)}{E'}{\Sigma'}
\end{equation*}
