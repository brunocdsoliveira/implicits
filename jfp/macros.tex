...
\newcommand{\myexruleR}[3]{\inferrule*[Right={\hyperlink{rule:#1}{\scriptsize\mylabel{#1}}}]{#2}{#3}\label{rule:#1}}
\newcommand{\myexruleT}[3]{\inferrule*[lab={\hyperlink{rule:#1}{\scriptsize\mylabel{#1}}}]{#2}{#3}\label{rule:#1}}
...
% algorithm
\newcommand{\alg}{\turns_{\mathrm{alg}}}
...
\newcommand{\admres}[8]{\hello{#1; #2; [#3] ~\gbox{\leadsto #4}; #5 \alg  #8; #6 ~\gbox{\leadsto #7}}}
\newcommand{\admress}[9]{\admres{#1}{#2}{#3}{#4}{#5}{#6}{#7}{#8}; #9}
\newcommand{\mgu}[3][\bar{\alpha}]{\hello{\mathrm{unify}_{\tenv;#1}(#2,#3)}}
\newcommand{\mgun}[4][\tenv]{\hello{\mathrm{unify'}_{#2}(#3,#4)}}
...
% coherent
\newcommand{\coh}{\turns_{\mathrm{sta}}}
...
\newcommand{\unambenv}[1]{\hello{\unambig{}{#1}}}

% annotated dmres
\newcommand{\dmresa}[7]{\hello{#1; #2; [#3]~\gbox{\leadsto #4} \ivturns #7; #5~\gbox{\leadsto #6}}}

%termination
\newcommand{\term}[1]{\hello{\turns_\mathrm{term} #1}}
\newcommand{\occ}[2]{\hello{\mathrm{occ}_{#1}(#2)}}
\newcommand{\tnorm}[1][\cdot]{\hello{\|#1\|}}
\newcommand{\head}[1]{\hello{\mathrm{hd}(#1)}}
\newcommand{\nterm}[1]{\hello{\not\turns_\mathrm{term} #1}}
...
% Remove unnecessary indentation in the first line after an equation-block
\newenvironment{myequation}{\begin{equation}}{\end{equation}\ignorespacesafterend}
\newenvironment{myequation*}{\begin{equation*}}{\end{equation*}\ignorespacesafterend}
