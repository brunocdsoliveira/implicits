...
for the head of the rule ($\rulet_2$). The rule \mylabel{R-TAbs}~wraps a type lambda binder 
...
In rule \mylabel{I-IAbs}~we need evidence for $\rulet_2$ to build evidence for $\type$, 
...
\begin{theorem}[Type-preserving translation]\label{thm:type:preservation} Let $e$ be an $\ourlang$
